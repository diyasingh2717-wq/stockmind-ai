...
# ── 2. SESSION STATE (INSTANT UNLOCK LOGIC) ───────
...
with col_left:
    ...
    if st.session_state.is_pro:
        ...
        if st.button("Run Pro Prediction →"):
            with st.spinner(f"Analyzing {stock.upper()}..."):
                try:
                    trend, prob, target = predict(stock.upper().strip())
                    color = "#22c55e" if trend == "UP" else "#ef4444"
                    st.success(f"PRO SIGNAL: {trend} | Confidence: {prob*100:.1f}% | Target: {target}")
                except Exception as e:
                    st.error(f"❌ {e}")
    else:
        st.warning("🔒 Features Locked. Activate Pro in the sidebar to unlock.")
        st.text_input("Stock Symbol", placeholder="Unlock Pro to search...", disabled=True)
...
# ── 7. SIDEBAR: PAYMENT & INSTANT ACTIVATION ──────
...
if not st.session_state.is_pro:
    ...
    with st.sidebar.form("payment_form"):
        ...
        utr_no = st.text_input("UPI Transaction ID (12-digits)")
        
        if st.form_submit_button("Activate My Pro Access"):
            if "@" in cust_email and len(utr_no) >= 12:
                try:
                    # Log to Supabase
                    db_entry = {"email": cust_email, "transaction_id": utr_no, "verified": False}
                ...
                st.sidebar.warning("Valid Email and 12-digit Transaction ID required.")
else:
